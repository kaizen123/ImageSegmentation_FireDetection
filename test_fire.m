...
function [result, outImg] = test_fire(inImg, count)
%close all hidden;
%delete('D:\workspace\matlab\ImageSegmentation_FireDetection\video\script\*')
output = 'D:\workspace\matlab\ImageSegmentation_FireDetection\dataset\output';
Im1 = inImg;
...
if (ff_r>( rows*columns*4/100) )
    ...
    %imshow(f_f)
    outImg = f_f;
    imwrite(f_f, fullfile(output, sprintf('%06d.jpg', count)));
    %title('fire detected');
    result = 1;
